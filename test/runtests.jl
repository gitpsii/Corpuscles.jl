# The tests are based on the test suite of the `particle` Python package from
...
@testset "catalogs" begin
    ...
    @test 0.054u"MeV/c^2" == Particle(553).width.value
    Corpuscles.use_catalog_file(joinpath(DATA_DIR, "particle2021.csv"))
    @test 0.054u"MeV/c^2" == Particle(553).width.value
    Corpuscles.use_catalog_file(joinpath(DATA_DIR, "particle2022.csv"))
    @test 0.054u"MeV/c^2" == Particle(553).width.value
end
